...
var dragonHeight		: float			= 0.0;
var dragonObject		: GameObject;

function Start(){
	if (dragonObject)
	{
		dragonHeight		= dragonObject.transform.position.y;
		transform.position.y		= Mathf.Clamp(transform.position.y + (0 * Time.deltaTime), minHeight, maxHeight) + dragonHeight;
	}
	FixTilt();
}
...
function Update(){
	...
	if (raising)
	{	
		transform.position.y		= Mathf.Clamp(transform.position.y + (riseSpeed * Time.deltaTime), minHeight + dragonHeight, maxHeight + dragonHeight);
		FixTilt();
	}
	if (lowering)
	{
...
function FixTilt(){
	var heightPercent				= (transform.position.y - (minHeight + dragonHeight)) / ((maxHeight + dragonHeight) - (minHeight + dragonHeight));
